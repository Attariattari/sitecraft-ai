...
const PERSONAL_INFO_KEY = "sitecraft_personal_info";
...
export function GenerateForm() {
    ...
    const loadData = async () => {
      try {
        ...
        if (userRes.ok) {
          ...
          // Get dynamic copy and recommended theme based on accountPurpose
          const purpose = getUserAccountPurpose(userData);
          const copy = getDashboardCopy(purpose);
          setDashboardCopy(copy);

          const recommendedThemes = getRecommendedThemesForPurpose(purpose);
          setRecommendedTheme(recommendedThemes[0] || "emerald");

          // Set default theme
          form.setValue("themeKey", recommendedThemes[0] || "emerald");
        }
      } catch (err) {
      ...
      try {
        ...
        if (saved) {
          ...
          // New structure uses selectedCategory
          const categoryId = parsed.selectedCategory || parsed.categoryId;
          if (
            categoryId &&
            parsed.data &&
  ...
  const onSubmit = async (data) => {
    ...
    const apiData = {
      ...
      colorMode: "dark",
      category: personalInfo?.categoryId || user?.accountPurpose || "portfolio",
      accountPurpose: user?.accountPurpose || "portfolio",
  ...
  return (
    <div className="w-full max-w-[1240px] mx-auto px-4 md:px-0 pb-20">
      ...
      <div className="mb-10 lg:mb-14">
        ...
        {!infoLoading && personalInfo && personalInfo.categoryId ? (
            ...
            <div>
              ...
              <p className="text-emerald-500/80 text-xs mt-1 font-medium">
                ...
                <span className="font-bold capitalize">
                  {personalInfo.categoryId}
                </span>{" "}
                info to pre-fill this form and give AI complete context.
              </p>
            </div>
            <Button
              type="button"
        ...
        <div className="lg:col-span-8">
          <form className="space-y-12">
            <AnimatePresence mode="wait">
              ...
              {currentStep === 0 && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormGroup
                      label={
                        user?.accountPurpose === "portfolio"
                          ? "Full Name"
                          : user?.accountPurpose === "ecommerce"
                            ? "Store Name"
                            : user?.accountPurpose === "salon"
                              ? "Salon Name"
                              : "Business / Brand Name"
                      }
                    ...
                    >
                      <Input
                        {...register("profession")}
                        placeholder="e.g. Senior Developer / Coffee Shop"
                        className="site-input"
                      />
                    </FormGroup>
                  </div>
                  ...
                  <FormGroup
                    label="Main Description / Details"
                    error={errors.bio?.message}
                    helper="Write a detailed description. Our AI will use this to generate all your website content and headlines."
                  >
                    <textarea
                      {...register("bio")}
                      className="site-textarea"
                      placeholder="Tell us about yourself, your business, or your store..."
                    />
                  </FormGroup>
              ...
              {/* STEP 2: PROJECTS / FEATURED ITEMS */}
              {currentStep === 1 && (
                <motion.div
                  key="stepProjects"
                  initial={{ opacity: 0, x: 20 }}
                ...
                >
                  ...
                  <Card className="p-8 bg-muted/20 border-border/40 rounded-3xl">
                    <div className="space-y-8">
                      <FormGroup label="Featured Item (Project, Product, Service)">
                        <Input
