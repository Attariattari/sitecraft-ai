...
export function getUserSelectedPurposes(user) {
    if (user && Array.isArray(user.selectedPurposes) && user.selectedPurposes.length > 0) {
...
export function getDashboardCopy(accountPurpose) {
    const config = getAccountPurposeConfig(accountPurpose);
    const label = config ? .label || accountPurpose;
    return (
...
export function getRecommendedThemesForPurpose(accountPurpose) {
    const purposeThemes = {
        portfolio: ["emerald", "modernDark", "minimalBlack", "premiumPurple", "cleanMint", "pureMonochrome"],
        business: ["warmBusiness", "corporateSlate", "emerald", "royalBlue", "financeEmerald"],
        salon: ["fashionRose", "creativeGradient", "luxuryGold", "premiumPurple", "emerald"],
        ecommerce: ["modernDark", "emerald", "royalBlue", "fashionRose", "creativeGradient"],
        restaurant: ["restaurantWarm", "luxuryGold", "warmBusiness", "natureOrganic", "pureMonochrome"],
        clinic: ["medicalClean", "cleanMint", "corporateSlate", "emerald", "natureOrganic"],
        realEstate: ["realEstateElite", "luxuryGold", "corporateSlate", "royalBlue", "warmBusiness"],
        agency: ["creativeStudio", "techNeon", "modernDark", "creativeGradient", "premiumPurple"],
        school: ["educationCalm", "corporateSlate", "royalBlue", "natureOrganic", "cleanMint"],
        landingPage: ["startupCoral", "techNeon", "emerald", "modernDark", "premiumPurple"],
    };
