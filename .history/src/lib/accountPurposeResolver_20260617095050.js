...
export function getUserAccountPurpose(user) {
    if (user ? .accountPurpose && ACCOUNT_PURPOSES.includes(user.accountPurpose)) {
    ...
    if (typeof window !== "undefined") {
        try {
            ...
            if (stored) {
                ...
                if (parsed.selectedCategory && ACCOUNT_PURPOSES.includes(parsed.selectedCategory)) {
...
export function getAccountPurposeConfig(accountPurpose) {
    const category = siteCraftPersonalInfoCategories.find((c) => c.id === accountPurpose);
    return category || siteCraftPersonalInfoCategories.find((c) => c.id === FALLBACK_PURPOSE);
}
