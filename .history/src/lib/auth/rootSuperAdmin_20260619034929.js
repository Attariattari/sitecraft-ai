...
export async function ensureRootSuperAdminUser() {
  ...
  if (user) {
    let dirty = false;
    if (user.role !== "super-admin") { user.role = "super-admin"; dirty = true; }
    if (!user.isRootSuperAdmin) { user.isRootSuperAdmin = true; dirty = true; }
    if (user.status !== "active") { user.status = "active"; dirty = true; }
