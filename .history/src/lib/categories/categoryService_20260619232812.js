...
export async function getAllCategories() {
    ...
    const categoriesWithUsage = await Promise.all(
        dbCategories.map(async(cat) => {
            const usageCount = await getCategoryUsage(cat.slug);
            return {
                ...cat,
                usageCount,
...
export async function getAvailableCategories(context = "home") {
    ...
    // All user-facing categories MUST be isActive
    ...
    const dbCategories = await Category.find(query)
        .sort({ isAvailable: -1, isLocked: 1, sortOrder: 1, name: 1 })
        .lean();

    // Transform categories to include isSelectable and displayStatus
    const transform = (cat) => ({
        ...cat,
        isSelectable: cat.isAvailable && !cat.isLocked,
        displayStatus: !cat.isAvailable ? "Unavailable" : cat.isLocked ? "Locked" : "Available",
        _id: cat._id ? .toString()
    });
    ...
    if (!dbCategories || dbCategories.length === 0) {
        ...
        const filtered = defaultCategories.filter(cat => {
            ...
            if (context === "signup") return cat.showInSignup;
            if (context === "dashboard") return cat.showInDashboard;
            return true;
        });

        return filtered.map(transform).sort((a, b) => {
            if (a.isSelectable !== b.isSelectable) return b.isSelectable ? 1 : -1;
            return a.sortOrder - b.sortOrder;
        });
    }

    return dbCategories.map(transform);
}

/**
 * Checks usage of a category in existing sites
 */
export async function getCategoryUsage(slug) {
    try {
        const Site = mongoose.models.Site || (await
            import ("@/models/Site")).default;
