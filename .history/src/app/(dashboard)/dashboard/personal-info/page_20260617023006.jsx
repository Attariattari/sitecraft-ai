      ...
      <div className="max-w-[1200px] mx-auto w-full pb-20">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            <span className="inline-flex items-center gap-1.5 bg-emerald-500/10 text-emerald-600 text-[11px] font-bold px-3 py-1 rounded-full border border-emerald-500/20 uppercase tracking-wider">
        ...
        {/* Prompt banner */}
        <div className="relative overflow-hidden bg-gradient-to-br from-emerald-500/8 via-card to-card border border-emerald-500/20 rounded-2xl p-6 mb-8">
          <div className="absolute -top-8 -right-8 w-36 h-36 rounded-full bg-emerald-500/10 blur-3xl pointer-events-none" />
          <div className="relative flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-muted-foreground mt-0.5">
                Pick a category — we show only the fields that matter for your
                website type.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {siteCraftPersonalInfoCategories.map((cat) => {
            const Icon = ICON_MAP[cat.icon] || User;
            const badgeClass =
              BADGE_COLORS[cat.badge] ||
              "bg-gray-500/15 text-gray-500 border-gray-500/20";
            const sectionCount = cat.sections?.length || 0;
            return (
              <button
                key={cat.id}
                type="button"
                onClick={() => setSelectedCategoryId(cat.id)}
                className="group relative flex flex-col text-left overflow-hidden rounded-[20px] bg-card hover:bg-card/80 transition-all duration-300 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 border border-transparent hover:border-emerald-500/20 hover:shadow-xl hover:shadow-emerald-500/5 ring-1 ring-border"
              >
                {/* Ambient Hover Glows */}
                <div className="absolute -top-24 -right-24 w-48 h-48 bg-emerald-500/20 blur-[50px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                <div className="absolute -bottom-24 -left-24 w-48 h-48 bg-emerald-500/10 blur-[50px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                ...
                <div className="relative p-6 flex flex-col h-full z-10">
                  ...
                  <div className="flex items-start justify-between mb-5">
                    <div className="relative">
                      ...
                      <div className="relative w-12 h-12 rounded-[14px] bg-gradient-to-b from-emerald-400/10 to-emerald-500/5 border border-emerald-500/20 flex items-center justify-center group-hover:border-emerald-500/40 group-hover:scale-110 transition-all duration-300 shadow-sm">
                        <Icon
                    ...
                    {cat.badge && (
                      <span
                        className={cn(
                          "uppercase text-[9px] font-black tracking-[0.08em] px-2.5 py-1 rounded-full border shadow-sm",
                          badgeClass,
                        )}
                      >
                        {cat.badge}
                  ...
                  <div className="flex-1">
                    <h3 className="text-[17px] font-bold text-foreground mb-1.5 group-hover:text-emerald-500 transition-colors duration-300">
                      {cat.label}
                    </h3>
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-[11px] font-semibold text-muted-foreground bg-muted/60 px-2 py-1 rounded-md border border-border/50">
                        {cat.purpose}
                      </span>
                      <span className="flex items-center gap-1.5 text-[11px] font-semibold text-muted-foreground">
                        <span className="w-1.5 h-1.5 rounded-full bg-border group-hover:bg-emerald-500 transition-colors" />
                        {sectionCount} sections
                      </span>
                    </div>

                    <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center opacity-0 -translate-x-3 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 border border-emerald-500/20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-[15px] font-semibold bg-emerald-600 hover:bg-emerald-700 text-white transition-colors shadow-md shadow-emerald-500/20"
          >
      ...
      <div className="bg-card border border-border rounded-2xl p-5 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-5 shadow-sm">
        ...
        <div className="flex-1">
          ...
          <p className="text-[13px] text-muted-foreground mb-3">
            Fill in your {selectedCategory?.label} details so SiteCraft AI can
            generate accurate, detailed websites for you.
          </p>
        ...
        {activeSection && (
          <div className="flex-1 bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm min-h-[400px]">
            ...
            <div className="space-y-6">
              {/* CASE 1: Whole section is a repeater (section.type === 'repeater') */}
                ...
                return (
                  <>
                    <button
                      ...
                      onClick={() =>
                        prevSection && setActiveSectionId(prevSection.sectionId)
                      }
                    ...
                    {nextSection ? (
                      ...
                      <button
                        type="button"
                        onClick={saveToLocalStorage}
                        className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-[14px] font-semibold bg-emerald-600 hover:bg-emerald-700 text-white transition-colors"
                      >
                        <Save className="w-4 h-4" />
                        Save Personal Info
                      </button>
