...
export default function PersonalInfoPage() {
  ...
  // Load from localStorage & Fetch user accountPurpose on mount
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed.categoryId) storageCategory = parsed.categoryId;
        if (parsed.data) storageData = parsed.data;
      }
    } catch {
      /* ignore */
    }
    ...
    const fetchUser = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Priority 1: User Database Purpose
          // Priority 2: LocalStorage Category
          // Priority 3: Default "portfolio"
          if (user?.accountPurpose) {
            setSelectedCategoryId(user.accountPurpose);
          } else if (storageCategory) {
            setSelectedCategoryId(storageCategory);
          } else {
        ...
        if (storageCategory) setSelectedCategoryId(storageCategory);
        else setSelectedCategoryId("portfolio");
      }

      setFormData(storageData); // Keep local data state, it will be mapped correctly for any form
      setHydrated(true);
    };
