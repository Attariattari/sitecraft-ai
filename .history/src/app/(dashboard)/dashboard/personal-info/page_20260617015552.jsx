      ...
      <div className="max-w-[1200px] mx-auto w-full pb-20">
        ...
        <div className="mb-10">
          ...
          <p className="text-muted-foreground text-[15px] mt-2">
            First, tell us what type of website you want to build.
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 mb-8 flex items-start gap-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-[14px] text-muted-foreground mt-1">
              Select a category to see relevant fields. SiteCraft AI will use
              this info to generate a personalized website.
            </p>
          </div>
        </div>
          ...
          {siteCraftPersonalInfoCategories.map((cat) => {
            ...
            return (
              <button
                ...
                className="group text-left bg-card border border-border rounded-2xl p-5 hover:border-emerald-500/50 hover:shadow-md hover:shadow-emerald-500/5 transition-all duration-200 hover:-translate-y-0.5"
              >
                ...
                <h3 className="text-[16px] font-bold text-foreground mb-1 group-hover:text-emerald-600 transition-colors">
                  {cat.label}
                </h3>
                <p className="text-[13px] text-muted-foreground leading-relaxed line-clamp-2">
                  {cat.description}
                </p>
                <div className="mt-4 flex items-center gap-1 text-[13px] font-medium text-emerald-600 opacity-0 group-hover:opacity-100 transition-opacity">
                  Select this
          ...
          <div className="flex-1 bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm min-h-[400px]">
            ...
            <div className="space-y-6">
              {activeSection.fields.map((field) => {
                if (field.type === "repeater") {
                  return (
                    <RepeaterField
                      key={field.name}
                      field={field}
                      value={formData?.[activeSection.sectionId]?.[field.name]}
                ...
                // Lay out in grid for regular fields
                const isFullWidth =
                  field.type === "textarea" || field.type === "switch";

                return (
                  <div key={field.name} className={isFullWidth ? "" : ""}>
                    <FieldRenderer
                      field={field}
                      value={getFieldValue(activeSection.sectionId, field.name)}
                      onChange={handleFieldChange}
