...
export default function PersonalInfoPage() {
  ...
  const [categoryChangeConfirm, setCategoryChangeConfirm] = useState(null);

  // Load from localStorage with per-category structure
  const loadStorageData = () => {
    ...
    return { selectedCategory: null, data: {} };
  };

  // Load user & initialize category on mount
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Use resolver: Priority 1 = user.accountPurpose, then localStorage, then default
          const userAccountPurpose = getUserAccountPurpose(data.user);
          const storageData = loadStorageData();

          // Initialize category
          setSelectedCategoryId(userAccountPurpose);
          // Load form data for this category
          if (storageData.data[userAccountPurpose]) {
            setFormData(storageData.data[userAccountPurpose]);
          }
        } else {
          // Fallback to localStorage
          const storageData = loadStorageData();
          const fallbackCategory = storageData.selectedCategory || "portfolio";
          setSelectedCategoryId(fallbackCategory);
          if (storageData.data[fallbackCategory]) {
            setFormData(storageData.data[fallbackCategory]);
          }
        }
      } catch (error) {
  ...
  const confirmCategoryChange = () => {
    ...
    const newCategoryId = categoryChangeConfirm.toCategory;

    // Save current category data before switching
    saveToLocalStorage(selectedCategoryId, formData);

    // Switch category
    setSelectedCategoryId(newCategoryId);

    // Load data for new category if exists
    const storageData = loadStorageData();
    if (storageData.data[newCategoryId]) {
      // Data exists for this category, load it
      const categoryData = storageData.data[newCategoryId];
      setFormData(categoryData);
    } else {
      // No data for this category yet, show empty form
      setFormData({});
    }
    ...
    setToast({
      type: "success",
      message: `Switched to ${getAccountPurposeConfig(newCategoryId)?.label || "this category"}`,
    });

    // Update server-side accountPurpose if user is logged in
    if (user) {
      fetch("/api/user/account-purpose", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ accountPurpose: newCategoryId }),
      }).catch((err) =>
        console.error("Failed to update accountPurpose on server:", err),
      );
    }
  ...
  const saveToLocalStorage = (
    ...
    try {
      ...
      // Update the specific category's data
      const updatedData = {
        ...storageData.data,
        [categoryId]: data,
      };

      // Save back with per-category structure
  ...
  const resetAll = () => {
    setFormData({});
    setSelectedCategoryId("portfolio");
    ...
    <div className="max-w-[1200px] mx-auto w-full pb-20">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-[15px] mt-2 max-w-xl">
          {selectedCategoryId
            ? getDashboardCopy(selectedCategoryId)?.personalInfoSubtitle ||
              "Keep your information up to date for better website generation."
            : "Your saved info powers AI-generated websites. Pick your website category to get started."}
        </p>
      </div>
      ...
      {categoryChangeConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6 animate-in fade-in scale-95">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-foreground text-lg">
                  Change Website Purpose?
                </h3>
                <p className="text-sm text-muted-foreground mt-1">
                  Switching to{" "}
                  <strong>
                    {getAccountPurposeConfig(categoryChangeConfirm.toCategory)
                      ?.label || "this category"}
                  </strong>{" "}
                  will update the form fields. Your saved data will not be
                  deleted, but different fields may be shown.
                </p>
              </div>
            </div>
            <div className="space-y-3">
              <button
                ...
                className="w-full px-4 py-2.5 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-sm transition-colors"
              >
                Yes, Switch Purpose
              </button>
              <button
                onClick={() => setCategoryChangeConfirm(null)}
                className="w-full px-4 py-2.5 rounded-lg border border-border bg-card hover:bg-muted text-foreground font-medium text-sm transition-colors"
              >
                Cancel
              </button>
            </div>
