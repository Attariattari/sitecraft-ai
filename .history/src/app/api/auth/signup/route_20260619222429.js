...
export async function POST(req) {
  try {
    ...
    // Validate account purpose availability
    const isSelectable = await isCategorySelectable(
      validatedData.accountPurpose,
    );
    if (!isSelectable) {
      return NextResponse.json(
        {
          success: false,
          message: "Selected category is not currently available.",
        },
        { status: 400 },
      );
    }
