...
export default function AdminCategoriesPage() {
  ...
  const handleSeed = async () => {
    const ok = await confirm({
      ...
      description:
        "This will create any missing default categories in the database. Existing records will not be overwritten.",
          ...
          >
            <div className="flex-1 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-border bg-muted/30 text-sm font-semibold outline-none focus:ring-2 focus:ring-primary/20"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-border bg-muted/30 text-sm font-semibold outline-none focus:ring-2 focus:ring-primary/20 h-24 resize-none"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 rounded-xl border border-border bg-muted/30 text-sm font-bold outline-none"
                      >
                ...
                <div className="space-y-6">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-primary border-b border-primary/20 pb-1 flex items-center gap-2">
                    <Zap className="w-3 h-3" /> Availability Control
                  </h4>
                  <div className="space-y-4">
                    <label className="flex items-center justify-between cursor-pointer p-3 rounded-2xl border border-border hover:bg-muted/50 transition-all">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-foreground">
                          Active for Users
                        </span>
                        <span className="text-[10px] text-muted-foreground">
                          Enable this category on platform
                        </span>
                      </div>
                      <button
                        onClick={() => {
                          const val = !editForm.isAvailable;
                          setEditForm({
                            ...editForm,
                            isAvailable: val,
                            // If unchecking available, automatically lock and hide
                            isLocked: !val,
                            showOnHome: val,
                            showInSignup: val,
                            showInDashboard: val,
                            lockedReason: !val
                              ? "Coming soon"
                              : editForm.lockedReason,
                          });
                        }}
                        className={cn(
                          "w-12 h-6 rounded-full transition-all relative border border-border shadow-inner",
                          editForm.isAvailable ? "bg-emerald-500" : "bg-muted",
                        )}
                      >
                        <div
                          className={cn(
                    ...
                    {editForm.isAvailable ? (
                      <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 space-y-4 animate-in fade-in zoom-in-95">
                        ...
                        {editForm.isLocked && (
                          <div className="space-y-1.5 animate-in slide-in-from-top-1">
                            ...
                            <select
                              value={editForm.lockedReason || "Coming soon"}
                              onChange={(e) =>
                                setEditForm({
                                  ...editForm,
                              ...
                              className="w-full px-3 py-2 rounded-xl border border-amber-500/20 bg-amber-500/5 text-xs font-bold outline-none"
                            >
                      ...
                      <div className="p-4 rounded-2xl bg-muted/50 border border-border border-dashed text-center">
                        <Lock className="w-6 h-6 mx-auto mb-2 text-muted-foreground opacity-20" />
                        <p className="text-xs font-bold text-muted-foreground">
                          Category is Restricted
                        </p>
                        <p className="text-[10px] text-muted-foreground/60 italic">
                          Locked and hidden by default
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-6">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-primary border-b border-primary/20 pb-1 flex items-center gap-2">
                    <Eye className="w-3 h-3" /> Visibility Targets
                  </h4>
                  <div
                    className={cn(
                  ...
                  >
                    ...
                    <div className="pt-2">
                      ...
                      <input
                        ...
                        className="w-16 px-3 py-1.5 rounded-lg border border-border bg-muted/30 text-sm font-bold"
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col justify-end gap-2 shrink-0 md:min-w-[160px]">
              <button
                ...
                className="px-4 py-2.5 rounded-xl border border-border font-bold text-sm hover:bg-muted transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveEdit}
                className="px-4 py-3 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:shadow-lg hover:shadow-primary/20 transition-all flex items-center justify-center gap-2"
              >
                <Save className="w-4 h-4" />
                Save Changes
              </button>
