...
export const realtimeEmitter = {
    ...
    async emitToUser(userId, event, payload = {}) {
        ...
        // Do NOT create a DB notification for FORCE_LOGOUT events —
        // those are used to invalidate sessions and relying on
        // token/sessionVersion mismatch (401) is sufficient. Creating
        // a notification here can cause the originating client to
        // receive a forced-logout notification immediately which
        // leads to unexpected self-logout behaviour.
        if (payload.title && payload.message && event !== REALTIME_EVENTS.SESSION.FORCE_LOGOUT) {
