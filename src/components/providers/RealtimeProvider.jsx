    ...
    if (user) {
      ...
      const bcHandler = (ev) => {
        ...
        if (data?.type === "logout") {
          ...
          try {
            const path = typeof window !== "undefined" ? window.location.pathname : "";
            // If user is on homepage, just update UI. Otherwise redirect to login.
            if (path && path !== "/") {
              router.push("/login");
            }
          } catch (e) {
        ...
        if (data?.type === "login") {
          // Update user context and refresh UI
          try {
          ...
          addToast({ title: "Signed in", message: "Signed in from another tab.", type: "success" });
        }
