...
function FAQItem({ faq, index, isOpen, toggleOpen }) {
  return (
    <motion.div
      ...
      className={`rounded-[2rem] border transition-all duration-300 overflow-hidden mb-4
        ${isOpen ? "border-primary/30 bg-primary/5 shadow-lg shadow-primary/5" : "border-border bg-soft hover:border-primary/20"}`}
    >
      <button
        onClick={toggleOpen}
      ...
      >
        ...
        <div
          className={`size-10 rounded-full flex items-center justify-center transition-all duration-300
            ${isOpen ? "bg-primary text-primary-foreground rotate-180" : "bg-background text-primary border border-primary/20"}`}
        >
